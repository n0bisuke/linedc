# LINEDC connpass 全イベント取得エージェント向け手順書

このリポジトリは、LINEDC の connpass グループページ（`https://linedevelopercommunity.connpass.com/event/`）からイベント情報をスクレイピングし、Markdown テーブルとして保存するための作業一式です。

## 目的

- connpass 上の **全イベント** を取得して `data/linedc_events.md` に書き出す
- connpass の一覧ページはグループによって最古ページ番号が変わるため、基本は **自動検出（`--start-page 0`）** する
- 取得は **最古ページ → 新しい方へ** の順で ID を採番（`1..`）
...
- 一部の実行環境では `python3` プロセスからの名前解決が制限されることがあるため、その場合は `scripts/fetch_linedc_raw.sh` で先にHTMLを取得してから `--raw-dir` で変換する。

## 実装の入口

- スクレイパー: `scripts/scrape_linedc_connpass.py`
- 出力: `data/linedc_events.md`
- スライド検証キャッシュ: `data/slide_url_cache.json`

## 出力フォーマット（Markdown テーブル）

`data/linedc_events.md` は以下の列で固定：

|列名|意味|
|---|---|
|id|最古イベントから順に採番（1,2,3...）|
|vol|タイトルから抽出した `vol.N`（抽出できない場合は空）|
|タイプ|大まかなカテゴリ（例: `Lunch Time Input` / `LT大会` / `ハンズオン` / `本体`）|
...
```sh
python3 scripts/scrape_linedc_connpass.py --limit 10000 --out data/linedc_events.md
```
